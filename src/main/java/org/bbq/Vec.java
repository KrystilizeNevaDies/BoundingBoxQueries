 ...
 */
public record Vec(double x, double y, double z) implements QueryItem {
    ...
    public double angle(@NotNull Vec vec) {
        final double dot = clamp(dot(vec) / (length() * vec.length()), -1.0, 1.0);
     ...
     * will be counterclockwise when the axis is Vecing towards the observer.
     * <p>
